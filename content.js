...
function cload() {
    ...
    load_logo.src = browser.runtime.getURL("/icon.svg");
...
function generate_share_image() {
    ...
    logo_img.src = browser.runtime.getURL("/logo.svg");
...
function launch_nsresult() {
    ...
    logo.src = browser.runtime.getURL("/logo.svg");
...
if (btn !== null) {
    ...
    if (document.getElementById("studentTermId").selectedIndex === document.getElementById("studentTermId").options.length-1) {
        browser.storage.local.set({"result_page": location.origin+location.pathname, "reports": parse_report(), "updated": Date.now()},function(){});
    }
}